...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      {/* SideNavBar (Desktop) */}
      <aside className="hidden md:flex flex-col p-5 lg:p-6 bg-white/95 dark:bg-[#0f1a2e]/95 text-slate-900 dark:text-slate-100 h-screen w-64 lg:w-72 shrink-0 sticky top-0 border-r border-[#DCE6F2] dark:border-[#203652] z-40 select-none shadow-2xs backdrop-blur-md transition-colors overflow-hidden">
        {/* Brand Header */}
        <div className="mb-6">
          <motion.button
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#6E8CB4] px-3 mb-1">
            Group
          </span>
          <nav
              ...
              return (
                <motion.button
                  ...
                  className={`group relative flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-sm font-medium transition-colors cursor-pointer ${
                    isActive || isHovered
                      ? 'text-[#16273F] dark:text-white font-semibold'
                ...
                >
                  ...
                  {isHighlighted && (
                    <motion.div
                      ...
                      className={`absolute inset-0 rounded-xl border transition-colors ${
                        isActive
                          ? 'bg-[#E7F0FB] dark:bg-[#203652] border-[#DCE6F2] dark:border-[#2A4365] shadow-2xs'
                          : 'bg-[#F7FAFD] dark:bg-[#203652]/60 border-[#DCE6F2]/60 dark:border-[#2A4365]/60'
                      }`}
                    />
                  )}
                  ...
                  <div className="flex items-center gap-2.5 relative z-10">
                    <div
                      className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all ${
                        isActive || isHovered
                          ? 'bg-white dark:bg-[#16273F] text-[#16273F] dark:text-white shadow-2xs'
                  ...
                  {isActive && (
                    <motion.span
                      ...
                      className="w-1.5 h-1.5 rounded-full bg-[#16273F] dark:bg-[#B4D0EE] shrink-0 relative z-10"
                    />
                  )}
                </motion.button>
              );
        ...
        {group && (
          <div className="flex flex-col mt-5 pt-4 border-t border-[#DCE6F2] dark:border-[#203652] min-h-0 flex-1">
            <div className="flex items-center justify-between px-2 mb-2">
              ...
              {onInviteFriends && (
                <motion.button
                  ...
                  className="text-[11px] font-semibold text-[#16273F] dark:text-[#B4D0EE] hover:underline flex items-center gap-1 cursor-pointer"
                  title="Invite or add friends to split"
                >
                  <UserPlus className="w-3 h-3 text-[#6E8CB4]" />
                  <span>Invite</span>
                </motion.button>
              )}
            </div>
                  ...
                  <motion.div
                    ...
                    className={`flex items-center justify-between py-1.5 px-2 rounded-xl transition-all cursor-pointer group ${
                      member.isCurrentUser
                        ? 'bg-[#E7F0FB]/80 dark:bg-[#203652]/80 border border-[#DCE6F2] dark:border-[#2A4365]'
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span className="text-xs font-medium text-[#16273F] dark:text-white truncate max-w-[85px] lg:max-w-[105px]">
                          {member.name}
                        </span>
                        {member.isCurrentUser && (
                          <span className="text-[8px] bg-[#16273F] dark:bg-white text-white dark:text-[#16273F] font-semibold px-1.5 py-0.2 rounded-full shrink-0">
                            You
                          </span>
                        )}
                      </div>
        ...
        <div className="mt-auto flex flex-col gap-2.5 pt-4 border-t border-[#DCE6F2] dark:border-[#203652]">
          ...
          <div className="flex items-center justify-between px-1">
            <div className="min-w-0 pr-2">
              ...
              <p className="text-xs font-medium text-[#16273F] dark:text-white truncate mt-0.5">
                {groupName}
              </p>
            </div>
          ...
          {onOpenShortcuts && (
            <motion.button
              ...
              className="flex items-center justify-between px-2 py-1 text-xs text-[#6E8CB4] hover:text-[#16273F] dark:hover:text-white transition-colors cursor-pointer rounded-xl hover:bg-[#F7FAFD] dark:hover:bg-[#203652]/60"
            >
              <div className="flex items-center gap-1.5">
                <Keyboard className="w-3.5 h-3.5" />
                <span className="text-[11px]">Shortcuts</span>
              </div>
              <kbd className="font-mono text-[9px] font-semibold bg-[#E7F0FB] dark:bg-[#203652] px-1.5 py-0.5 rounded border border-[#DCE6F2] dark:border-[#2A4365] text-[#16273F] dark:text-[#B4D0EE]">
                ?
              </kbd>
            </motion.button>
          )}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.96 }}
            onClick={onNewGroup}
            className="w-full flex items-center justify-center gap-2 py-2 px-4 bg-[#16273F] dark:bg-white text-white dark:text-[#16273F] rounded-full font-semibold text-xs tracking-wide hover:opacity-90 transition-opacity cursor-pointer shadow-2xs"
          >
      ...
      {/* Mobile Top App Bar */}
      <header className="md:hidden flex justify-between items-center w-full px-4 py-3 bg-white/95 dark:bg-[#0f1a2e]/95 backdrop-blur-md sticky top-0 z-40 border-b border-[#DCE6F2] dark:border-[#203652] shadow-2xs transition-colors">
        <button
          onClick={onBackToLanding}
          className="flex items-center gap-2.5 text-left cursor-pointer"
        >
          <Logo size={28} />
          <span className="font-serif-display text-lg text-[#16273F] dark:text-white tracking-tight lowercase">
            nooswise
          </span>
        </button>
        ...
        <div className="flex items-center gap-2">
          {onOpenShortcuts && (
            <motion.button
              ...
              className="w-8 h-8 rounded-full bg-[#E7F0FB] dark:bg-[#203652] text-[#16273F] dark:text-[#B4D0EE] flex items-center justify-center cursor-pointer"
            >
              <Keyboard className="w-3.5 h-3.5" />
            </motion.button>
          )}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={onNewGroup}
            className="flex items-center gap-1 bg-[#16273F] dark:bg-white text-white dark:text-[#16273F] text-xs font-semibold px-3 py-1.5 rounded-full hover:opacity-90 transition-opacity shadow-2xs"
          >
      ...
      <nav
        ...
        className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-5 pt-2 md:hidden bg-white/95 dark:bg-[#0f1a2e]/95 backdrop-blur-lg border-t border-[#DCE6F2] dark:border-[#203652] shadow-[0_-4px_24px_rgba(0,0,0,0.06)] dark:shadow-[0_-4px_24px_rgba(0,0,0,0.3)] transition-colors"
      >
        {navItems.map((item) => {
          const isActive = activeTab === item.id;
              ...
              {isHighlighted && (
                <motion.div
                  ...
                  className={`absolute inset-0 rounded-2xl border transition-colors ${
                    isActive
                      ? 'bg-[#E7F0FB] dark:bg-[#203652] border-[#DCE6F2] dark:border-[#2A4365] shadow-2xs'
                      : 'bg-[#F7FAFD] dark:bg-[#203652]/70 border-[#DCE6F2]/60 dark:border-[#2A4365]/60'
                  }`}
                />
              )}
